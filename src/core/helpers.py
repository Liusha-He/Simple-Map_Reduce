...
from typing import Text, Dict, List, Set, Iterable
...
def append_inverted_index(word2docs: Dict[Text, List],
                          parsed_doc: Dict[int, Set]) -> Dict[Text, List]:
...
def get_word2idx(word2idx: Dict[Text, int], idx: int, parsed_doc: Dict[int, Set]) -> (Dict[Text, int], int):
...
def get_word2doc_id_mapping(parsed_doc: Dict[int, Set], word2idx: Dict[Text, int]):
    ...
    return [(word2idx[word], i) for i, wordset in parsed_doc.items() for word in wordset]


def get_wordid2docid_mapping(parsed_doc: Dict[int, Set], word2idx: Dict[Text, int]) -> Dict[int, int]:
    """"""
    wordid2docid = {}
    for doc_id, words in parsed_doc.items():
        for w in words:
...
def map_wordid_docs(word2docs: Dict[Text, List], word2idx: Dict[Text, int]) -> Dict[int, List[int]]:
    """"""
